...
<h1 class="text-center">NUEVO ESTUDIANTE</h1>
<form  class="formulario" action="Guardar/g_estudiante.php" method="post" >
...
<div class="card-body ">
        ...
        <select  class="custom-select  mb-4 " name="Programa"  autofocus  >
           ...
           while($fila=$sql->fetch_array()){
             echo "<option value='".$fila['Id_Programa']."'>".$fila['Nombre_p']." ".$fila['Modalidad']."</option>";
           }           
           ?>
        </select> 
        <span>Sede:</span>
        ...
        <select  class="custom-select  mb-4 " name="Rol"  autofocus  value="<?php echo $datos['R_admin']; ?>">
           <?php 
           $sql=$miconexion->query("SELECT * FROM `Role` WHERE id_rol = 3");
           while($fila=$sql->fetch_array()){
